...
class MisspelledSentenceSearcher:
    ...
    def __init__(self, data: Data, complete_sentence_searcher: CompleteSentenceSearcher):
        ...
        self.__editors = self.create_editors()
    ...
    def edit_sentences(self, user_input: str,
        ...
        for line in intersection_of_lines:
            ...
            first_match_index = self.find_first_match(split_sentence, prefix)
            ...
            if self.check_pattern_match(first_match_index, split_sentence, prefix) and \
                ...
                for editor in self.__editors:
                    editing_place = editor.edit(misspelled_word.misspelled_word, split_sentence[real_word_index])
                    if editing_place != -1:
                        edit = True
                        score = editor.rank(user_input, misspelled_word.misspelled_word_place, editing_place)
    ...
    @staticmethod
    def check_pattern_match(index: int, complete_sentence: list[str], partial_input: list[str]) -> bool:
        """
    ...
    def create_editors():
        ...
        """
        list_editors = list()
        list_editors.append(EditAndRank(DeleteLetter()))
        list_editors.append(EditAndRank(AddLetter(DeleteLetter())))
        list_editors.append(EditAndRank(ChangeLetter()))
        return list_editors

    @staticmethod
    def find_first_match(split_sentence: list[str], prefix: list[str]):
        """
